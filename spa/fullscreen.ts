...
var prefixes = ["", "webkit", "moz", "o", "ms", "khtml"],
    nativeRequest = ["requestFullscreen", "requestFullScreen"],
    prefixRequest = ["RequestFullscreen", "RequestFullScreen"],
    nativeCancel = ["cancelFullscreen", "cancelFullScreen", "exitFullscreen"],
    prefixCancel = ["CancelFullscreen", "CancelFullScreen", "ExitFullscreen"];
...
if (api.isSupported === true) {
    if (api.isIE) {
        ...
        api.isFullScreen = function () { return isFullScreen; };
    }
    else {
        ...
        if (event.check("fullscreenchange"))
            api.eventName = "fullscreenchange";
        else if (event.check(api.prefix + "fullscreenchange"))
            api.eventName = api.prefix + "fullscreenchange";
    }
}

ko.bindingHandlers.fullscreen = {
    init: function (element: HTMLElement, valueAccessor: () => any, allBindingsAccessor: () => {}, viewModel: any, bindingContext: KnockoutBindingContext): void {
        if (api.isSupported) {
            var onClick = function () {
                var options = ko.utils.unwrapObservable(valueAccessor()),
                    icon = ko.utils.unwrapObservable(options.icon),
                    cancelIcon = ko.utils.unwrapObservable(options.cancelIcon),
                    isFullScreen = ko.utils.unwrapObservable(options.isFullScreen);
